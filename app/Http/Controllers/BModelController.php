...
class BModelController extends Controller
{
    ...
    public function store(StoreBModelRequest $request, PostHogService $posthog)
    {
        ...
        $posthog->capture($request->user()->email, 'bpm_model_created', [
            'model_id' => $model->id,
     ...
     */
    public function destroy(Request $request, string $id, PostHogService $posthog)
    {
        $model = BModel::find($id);
        $modelName = $model->name;
        $model->delete();

        $posthog->capture($request->user()->email, 'bpm_model_deleted', [
            'model_id' => $id,
